...
//            }
    ...
    func finishWriting() {
        ...
        self.writerInputs
            .lazy
            .filter { $0.value.isReadyForMoreMediaData }
            .forEach { $0.value.markAsFinished() }
        
        let status = self.writer?.status
        guard status == .writing else {
            self.delegate?.recorder(self, errorOccured: .failedToFinishWriting(error: self.writer?.error))
            return
        }
        group.enter()
        ...
        self.writer?.finishWriting { [weak self] in
            ...
            self.delegate?.recorder(self, finishWriting: self.writer!)
            self.writer = nil
