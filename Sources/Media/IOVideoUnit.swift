...
final class IOVideoUnit: NSObject, IOUnit {
    ...
    public var openPixelBuffer: CVPixelBuffer?
    ...
    @inline(__always)
    func metalEffect(_ buffer: CVImageBuffer, info: CMSampleBuffer?) -> CVPixelBuffer {
        var outputBuffer:CVPixelBuffer = buffer
        for effect in effects {
            outputBuffer = effect.metalExecute(buffer, info: info)
        }
        return outputBuffer
    }
    ...
    func appendSampleBuffer(_ sampleBuffer: CMSampleBuffer) {
        autoreleasepool {
            ...
            {
                // change render effect by metal
                // imageBuffer = metalEffect(buffer, info: sampleBuffer)
                ...
                if !effects.isEmpty {
                    ...
                    if buffer.width != Int(extent.width) || buffer.height != Int(extent.height) {
                        ...
                        print("Extend width = \(extent.width) height = \(extent.height)")
                        print("Buffer width = \(buffer.width) heigh = \(buffer.height)")
                        // imageBuffer = createPixelBuffer(width: Int(extent.width), height: Int(extent.height))
                        
                        // pixelBufferPool?.createPixelBuffer(&imageBuffer)
                        
                        // imageBuffer = createPixelBufferFrom(image: image)
//                        if (imageBuffer != nil) {
//                            print("convert pixel buffer ok")
//                        }
                        // resize buffer not use  create new buffer ==> out of memory error
                        // buffer.unlockBaseAddress()
                        
                        //                    //imageBuffer = buffer.resizePixelBuffer(buffer, width: Int(extent.width), height: Int(extent.height))
                        //                   // imageBuffer = buffer.createPixelBuffer(width: Int(extent.width), height: Int(extent.height))
